...
import { QrCode, UserPlus, X, Check, Copy } from "lucide-react";
...
const CheckInTab = ({ eventId, token, isExpired, onCheckin }: CheckInTabProps) => {
  ...
  const startScanner = () => {
    ...
    setTimeout(async () => {
      try {
        const scanner = new Html5Qrcode("checkin-qr-scanner");
  ...
  if (isExpired) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground font-sans">Party has ended. Check-in is closed.</p>
      </div>
    );
  }
        ...
        {isScanning ? (
          <div className="space-y-4">
            <div
              ...
              className="w-full max-w-sm mx-auto rounded-xl overflow-hidden bg-black/50 min-h-[300px]"
            />
            <Button
              variant="outline"
              onClick={stopScanner}
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-primary mb-2 font-sans font-medium">Phone (optional)</label>
              <Input
                value={walkInPhone}
          ...
          <div className="space-y-3">
            ...
            <Button
              ...
              className="w-full border-primary text-primary hover:bg-primary hover:text-primary-foreground h-14 text-base"
            >
      ...
      <Dialog open={!!walkInPassData} onOpenChange={(open) => !open && setWalkInPassData(null)}>
        <DialogContent className="sm:max-w-md bg-background border-border">
          ...
          <div className="flex flex-col items-center space-y-4 py-4">
            ...
            <p className="text-muted-foreground text-sm text-center font-sans">Hand this to the fan to scan</p>
      ...
      <Dialog open={showWalkInsQR} onOpenChange={setShowWalkInsQR}>
        <DialogContent className="sm:max-w-md bg-background border-border">
          ...
          <div className="flex flex-col items-center space-y-6 py-6">
            ...
            <p className="text-muted-foreground text-sm text-center font-sans">Fans scan to join the After Party</p>
            <Button
