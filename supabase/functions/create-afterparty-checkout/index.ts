...
serve(async (req) => {
  ...
  try {
    ...
    if (discountAmount === 100 && validatedCode) {
      ...
      // Send MailerLite notification (non-blocking) if confirmation email provided
      if (formData.confirmation_email) {
        sendMailerLiteNotification(
          event.id,
          formData.confirmation_email,
          token,
      ...
      return new Response(JSON.stringify({ 
        ...
        email_sent: !!formData.confirmation_email
      }), {
